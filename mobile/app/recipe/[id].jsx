...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        ...
        // CRITICAL FRONT-END FIX: Check if the response is an array before using .some()
        if (Array.isArray(favorites)) {
            ...
            console.warn("Favorites response was not an array:", favorites);
            setIsSaved(false);
        }
  ...
  const handleToggleSave = async () => {
    if (!userId || !recipeId || !recipe) {
      Alert.alert("Error", "Missing user or recipe data. Cannot save.");
      return;
    }
    ...
    try {
      if (isSaved) {
        ...
        setIsSaved(false);
        Alert.alert("Success", `${recipe.title} removed from favorites!`);
      } else {
        // add to favorites
  ...
  if (!recipe) {
    return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ fontSize: 18, color: COLORS.text }}>Recipe not found or failed to load.</Text>
